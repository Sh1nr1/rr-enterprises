              ...
              <motion.a
                className={`text-sm font-medium ${
                  index === items.length - 1
                    ? 'text-lime-400 cursor-default' // Changed breadcrumb accent color
...
// --- AgriculturalInnovationCenterPage Component ---
export default function AgriculturalInnovationCenterPage() {
  ...
  // Sample project images for agricultural innovation center
  const projectImages = [
    'https://images.unsplash.com/photo-1704452419411-cf0479b9badc?q=80&w=1974&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    'https://images.unsplash.com/photo-1714070705532-3f81f7733a1b?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    'https://images.unsplash.com/photo-1686008674009-876c599f1fe9?q=80&w=2127&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    'https://plus.unsplash.com/premium_photo-1678911006091-e89c632f049a?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'
  ];
  ...
  const themeClasses = mounted && isDarkMode
    ...
    : 'bg-gradient-to-br from-green-50 via-white to-green-100 text-gray-900'; // Changed background gradient for agri theme

  const cardClasses = mounted && isDarkMode
    ? 'bg-slate-800/40 border-slate-700/50 hover:bg-slate-800/60'
    : 'bg-white/40 border-green-200/50 hover:bg-white/60'; // Changed card border for agri theme
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <motion.div
          ...
          className={`absolute -top-40 -right-40 w-80 h-80 rounded-full ${
            mounted && isDarkMode ? 'bg-gradient-to-br from-lime-500/10 to-green-500/5' : 'bg-gradient-to-br from-lime-500/5 to-green-500/3'
          } blur-3xl`}
        />
        <motion.div
          animate={{
          ...
          className={`absolute -bottom-40 -left-40 w-96 h-96 rounded-full ${
            mounted && isDarkMode ? 'bg-gradient-to-br from-emerald-500/10 to-teal-500/5' : 'bg-gradient-to-br from-emerald-500/5 to-teal-500/3'
          } blur-3xl`}
        />
      </div>
      ...
      <div className="relative z-10 container mx-auto px-6 py-12 pt-28">
        ...
        >
          <Breadcrumb items={[
            ...
            { label: 'Bhopal Agricultural Innovation Center', href: '/projects/bhopal_agricultural_innovation_center' },
          ]} />
        </motion.div>
        ...
        >
          ...
          >
            A cutting-edge research and development facility for AgriTech Foundation, fostering sustainable agricultural practices.
          </motion.p>
        </motion.header>
            ...
            <div className="relative h-96 md:h-[500px] rounded-2xl overflow-hidden">
              <AnimatePresence initial={false} mode="wait">
                <motion.img
                  ...
                  alt={`Agricultural Project ${currentImage + 1}`}
                ...
                {projectImages.map((_, index) => (
                  <motion.button
                    ...
                    className={`w-3 h-3 rounded-full transition-all ${
                      currentImage === index ? 'bg-lime-400 scale-125' : 'bg-white/50' // Changed dot color
                    }`}
        ...
        >
          <div className="grid md:grid-cols-2 gap-8">
            <motion.div
              ...
              whileHover={{ scale: 1.01, boxShadow: mounted && isDarkMode ? "0 0 40px rgba(144,238,144,0.4)" : "0 0 30px rgba(144,238,144,0.2)" }}
            >
              ...
              <div className="space-y-4">
                <div className={`flex items-center space-x-3 ${mounted && isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>
                  <MapPin className="w-5 h-5 text-green-400" /> {/* Changed icon color */}
                  <span className="text-lg">Bhopal, Madhya Pradesh, India</span>
                </div>
                <div className={`flex items-center space-x-3 ${mounted && isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>
                  <Users className="w-5 h-5 text-lime-400" /> {/* Changed icon color */}
                  <span className="text-lg">AgriTech Foundation</span>
                </div>
                <p className={`text-lg leading-relaxed ${mounted && isDarkMode ? 'opacity-80' : 'opacity-90'}`}>
                  Development of a **25-acre Agricultural Innovation Center** focused on advanced farming techniques,
                  crop research, and sustainable resource management. The facility includes **hydroponics labs,
                  demonstration farms, and a learning center** for local farmers.
                </p>
              </div>
            </motion.div>
            ...
            <motion.div
              className={`p-8 rounded-3xl border transition-all duration-300 ${cardClasses} ${glowClasses}`}
              whileHover={{ scale: 1.01, boxShadow: mounted && isDarkMode ? "0 0 40px rgba(124,252,0,0.4)" : "0 0 30px rgba(124,252,0,0.2)" }}
            >
              ...
              <div className="grid grid-cols-2 gap-6">
                <motion.div className="text-center" initial={{ scale: 0.8 }} animate={{ scale: 1 }} transition={{ delay: 0.9, type: "spring" }}>
                  <div className="flex items-center justify-center mb-2">
                    <Sprout className="w-7 h-7 text-lime-400" /> {/* Changed icon */}
                  </div>
                  <div className={`text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r ${mounted && isDarkMode ? 'from-lime-300 to-green-400' : 'from-lime-600 to-green-700'}`}>25</div>
                ...
                <motion.div className="text-center" initial={{ scale: 0.8 }} animate={{ scale: 1 }} transition={{ delay: 1.0, type: "spring" }}>
                  <div className="flex items-center justify-center mb-2">
                    <Calendar className="w-7 h-7 text-emerald-400" /> {/* Changed icon color */}
                  </div>
                  <div className={`text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r ${mounted && isDarkMode ? 'from-emerald-300 to-lime-400' : 'from-emerald-600 to-lime-700'}`}>12</div>
                ...
                <motion.div className="text-center" initial={{ scale: 0.8 }} animate={{ scale: 1 }} transition={{ delay: 1.1, type: "spring" }}>
                  <div className="flex items-center justify-center mb-2">
                    <Tractor className="w-7 h-7 text-gray-400" /> {/* Changed icon */}
                  </div>
                  ...
                  <div className={`text-sm ${mounted && isDarkMode ? 'opacity-70' : 'opacity-80'}`}>Researchers & Staff</div>
                </motion.div>
                <motion.div className="text-center" initial={{ scale: 0.8 }} animate={{ scale: 1 }} transition={{ delay: 1.2, type: "spring" }}>
                  <div className="flex items-center justify-center mb-2">
                    <Lightbulb className="w-7 h-7 text-blue-400" /> {/* Changed icon */}
                  </div>
                  <div className={`text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r ${mounted && isDarkMode ? 'from-blue-300 to-cyan-400' : 'from-blue-600 to-cyan-700'}`}>3</div>
                  <div className={`text-sm ${mounted && isDarkMode ? 'opacity-70' : 'opacity-80'}`}>Labs</div>
                </motion.div>
              </div>
            </motion.div>
          </div>
        ...
        >
          <div className={`p-8 rounded-3xl border transition-all duration-300 ${cardClasses} ${glowClasses}`}>
            ...
            <div className="relative h-96 rounded-2xl overflow-hidden">
              <img
                src="https://images.unsplash.com/photo-1709472817735-201c813b1d41?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D" // Placeholder for "Before"
              ...
              />
              <img
                src="https://plus.unsplash.com/premium_photo-1678911006091-e89c632f049a?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D" // Placeholder for "After"
        ...
        >
          <div className={`p-8 rounded-3xl text-center border transition-all duration-300 ${cardClasses} ${glowClasses}`}>
            <div className="mb-6">
              <motion.div
                className="w-16 h-16 mx-auto rounded-full bg-gradient-to-r from-lime-400 to-green-500 flex items-center justify-center mb-4 text-white"
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
              ...
              <h4 className={`text-xl font-semibold ${mounted && isDarkMode ? 'text-white' : 'text-gray-900'}`}>Dr. Sandeep Malik</h4>
            ...
            <div className="flex justify-center space-x-1">
              {[...Array(5)].map((_, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.5 + i * 0.1, duration: 0.3 }}>
                  <Sun className="w-6 h-6 text-lime-400 fill-current" />
                </motion.div>
              ))}
            </div>
          </div>
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <p className={`text-lg mb-8 max-w-2xl mx-auto ${mounted && isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>
              Explore more of our impactful agricultural projects or delve deeper into our offerings.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl mx-auto">
              <Link href="/projects/smart-farming-initiative" passHref legacyBehavior>
                <Button className={`w-full py-3 px-6 rounded-xl text-lg font-semibold transition-all duration-300 transform ${mounted && isDarkMode ? 'bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-700 hover:to-green-700 text-white shadow-lg' : 'bg-gradient-to-r from-emerald-500 to-green-500 hover:from-emerald-600 hover:to-green-600 text-white shadow-lg'}`}
                  asChild
                >
                  <motion.a whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}><Tractor className="mr-2" /> View Our Smart Farming Project</motion.a>
                </Button>
              </Link>
              <Link href="/projects/irrigation-system-upgrade" passHref legacyBehavior>
                <Button className={`w-full py-3 px-6 rounded-xl text-lg font-semibold transition-all duration-300 transform ${mounted && isDarkMode ? 'bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-700 hover:to-blue-700 text-white shadow-lg' : 'bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white shadow-lg'}`}
                  asChild
                >
                  <motion.a whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}><Zap className="mr-2" /> Explore Irrigation Upgrades</motion.a>
                </Button>
              </Link>
              <Link href="/services/epc-agriculture" passHref legacyBehavior>
                <Button className={`w-full py-3 px-6 rounded-xl text-lg font-semibold transition-all duration-300 transform ${mounted && isDarkMode ? 'bg-gradient-to-r from-yellow-600 to-orange-600 hover:from-yellow-700 hover:to-orange-700 text-white shadow-lg' : 'bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-600 hover:to-orange-600 text-white shadow-lg'}`}
                  asChild
                >
                  <motion.a whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}><FlaskConical className="mr-2" /> Learn About Our Agri-EPC Services</motion.a>
                </Button>
              </Link>
